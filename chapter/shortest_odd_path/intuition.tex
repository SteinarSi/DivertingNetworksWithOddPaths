...
Even though it is deceptively simple to state the problem, solving it in practice is no easy task. In fact, \cite{source:even-cycles-in-directed-graphs} have shown that the problem is NP-complete in directed graphs, even without weights. Undirected graphs are also a challenge: \cite{source:odd-paths-cycles-and-tjoins} have proved that even if the weights are restricted to just $\{-1,1\}$, and even if we know that they do not form any negative cycles, the problem is still NP-complete in undirected graphs. 
