...
\begin{itemize}
    \item It is difficult to alternate through mirror and normal edges, while simultaneously alternating between computing two separate paths.
    \item The paths may not be of equal length, even if the graph is unweighted.
    \item The endpoints we start backtracking from may already be the base, if the blossom edge itself is adjacent it.
...
Here is our solution. The procedure returns the base and two lists of all the normal edges that make up blossom. That usually includes the blossom edge itself, which is part of both paths unless it is adjacent to the base itself.
...
\end{lstlisting}
See \Cref{appendix:full-pseudocode} for the full version.
...
Now we can update a base in constant time, with the tradeoff of potentially slower queries. Is this faster? Sometimes it is. We benchmark both versions and discuss the results in \Cref{subsubsection:testing-basis}.
