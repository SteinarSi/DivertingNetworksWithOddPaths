...
Now that we know what a graph is, we are ready to formalize the underlying problem of the example we started with in the previous \Cref{section:graphs}. Both problems can be represented by an abstract graph where we want to find the shortest path from one vertex to another. From a computational perspective, it does not matter whether the edges are roads or flights, or whether the vertices are crossroads or airports. Vertices and edges can represent whatever we want them to. We call the problem \textsc{Shortest Path}, and it is defined as:

\noindent\fbox{\parbox{0.94\textwidth}{\textsc{Shortest Path}\\
    \textbf{Input:} a graph $G$, two vertices $s,t \in V$\\
    \textbf{Output:} am $s$-$t$-path in $G$ of minimum cost
}}
...
\noindent\fbox{\parbox{0.94\textwidth}{\textsc{Shortest Odd Path}\\
    \textbf{Input:} a graph $G$, two vertices $s,t \in V$\\
    \textbf{Output:} am $s$-$t$-path in $G$ of minimum cost, that uses an odd number of edges
}}
