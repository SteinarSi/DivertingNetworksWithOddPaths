...
This is the main idea for our algorithm. See \Cref{subsection:network-diversion-example} for a run-through of the algorithm on a small example, and see \Cref{subsection:network-diversion-complexity} for a proof of its polynomial running time. Note that we did not come up with this idea ourselves, but have to thank \cite{source:pål} for his as for now unpublished work on the subject.
